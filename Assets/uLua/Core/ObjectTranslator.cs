...
//			enumFromIntFunction = new LuaCSFunction(this.enumFromInt);
		...
		internal void throwError (IntPtr luaState, object e)
		{
			...
			string errLocation = "";
			if (curlev.Length > 0) errLocation = curlev [0].ToString ();
		...
		public static int importType (IntPtr luaState)
		{
			...
			if (klass != null) translator.pushType (luaState, klass);
		...
		public static int registerTable (IntPtr luaState)
		{
			...
			if (LuaDLL.lua_type (luaState, 1) == LuaTypes.LUA_TTABLE)
			{
				...
				if (superclassName != null)
				{
					...
					else translator.throwError (luaState, "register_table: can not find superclass '" + superclassName + "'");
				}
				else translator.throwError (luaState, "register_table: superclass name can not be null");
			}
		...
		public static int getMethodSignature (IntPtr luaState)
		{
			...
			for (int i = 0; i < signature.Length; i++) signature [i] = translator.FindType (LuaDLL.lua_tostring (luaState, i + 3));
			try
			{
		...
		public static int getConstructorSignature (IntPtr luaState)
		{
			...
			for (int i = 0; i < signature.Length; i++) signature [i] = translator.FindType (LuaDLL.lua_tostring (luaState, i + 2));
			try
			{
		...
		internal object[] popValues (IntPtr luaState, int oldTop, Type[] popTypes)
		{
			...
			else
			{
				int iTypes;
				ArrayList returnValues = new ArrayList ();
				if (popTypes [0] == typeof(void)) iTypes = 1;
				else iTypes = 0;
				for (int i = oldTop + 1; i <= newTop; i++)
				{
